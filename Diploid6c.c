...
//#include "nrutil.h"
/* compile using 
 * gcc -g hickeyDiploid6c.c  poidev.c ran1.c -lm -o hickeyDiploid6c
 *
...
double arandom;
long above;
double fit_asexual, max_asexual, rel_asexual;
double fit_sexual, max_sexual, min_sexual, rel_sexual;
double min, max, max_fitness;
long no_asexual;
...
// *****   initialize   *******************************************
    ...
    sumSquares = sqrt((sumSquares - no_sexual*fit_sexual*fit_sexual)/(no_sexual-1.0));
...
//  fprintf(stdout,"  with %5d asexuals with avg fitness of %10.5f versus ,\n",no_asexual,fit_asexual);
    ...
    fprintf(stdout," min    Alleles      rel         rel \n");
    fprintf(stdout,"gen popSize asexuals asexualFit  sexualFit  asexualFit sexualFit ");
    fprintf(stdout,"sexualFit  perInd   asexualFit  sexualFit stddev\n");
    //---------------------------------------------
	...
	for(i=0, j=0; i<numberInd; i++) {
		...
		for(l=0; l<k; l++) {
		    ...
		    m[j++]=tmp;
                ...
                // fprintf(stdout,"Found asexual %5d\n",i);
		arandom = poidev(FECUNDITY*n[i].relativeFitness,iseed); 
		k = (long) arandom; 
	...
	//  fprintf(stdout,"                       avg        avg        max        max   \n");
        //  fprintf(stdout,"gen popSize asexuals asexualFit sexualFit asexualFit sexualFit\n");
	fprintf(stdout,"%3d  %6d  %6d %10.5f %10.5f %10.5f %10.5f %10.5f",gen,numberInd,no_asexual,fit_asexual,fit_sexual,max_asexual,max_sexual,min_sexual);
...
void calculateFitness(void) { // Calculate avg asexual and sexual fitness
	...
	if(n[i].sex) {   // only if asexual=1
	    ...
	    if(n[i].fitness < min_sexual) min_sexual = n[i].fitness;
