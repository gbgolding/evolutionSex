...
//#include "nrutil.h"
/* compile using 
 * gcc -g sexual7.c  poidev.c ran1.c -lm -o sexual7
 */
...
//    
	...
	for(j=0; j<NO_LOCI; j++) {
	    ...
	    n[i].fitness=n[i].fitness*(1.0+ (double)count*0.02);  // multiplicative fitness; dominance
            // if(i==14382 && count > 0) {
            //     fprintf(stdout,"%6d %6d %6d %10.5f\n",i,j,count,n[i].fitness);
            // }
	}
    }
...
//    
	...
	for(i=0; i<numberInd; i++) {
	    if(n[i].sex==0) { // sexual
		k=(int) (NO_LOCI-1)*ran1(iseed); // choose a spot for recomb
		for(j=k; j<NO_LOCI; j++) {
		    tmp_allele=n[i].paternal_allele[j];
		    n[i].paternal_allele[j]=n[i].maternal_allele[j];
		    n[i].maternal_allele[j]=tmp_allele;
		}
	    }
	}
	//
